<script>
	...
	import { createNewForm, getForms } from '$lib/apis/forms';
	...
	const submitHandler = async () => {
		...
		if (validateCommandString(command)) {
			const form = await createNewForm(localStorage.token, command, title, content).catch(
				(error) => {
					toast.error(error);
	...
	onMount(async () => {
		window.addEventListener('message', async (event) => {
			if (
				...
				return;
			const form = JSON.parse(event.data);
			console.log(form);
		...
		if (sessionStorage.form) {
			const form = JSON.parse(sessionStorage.form);

			console.log(form);
			title = form.title;
			await tick();
			content = form.content;
			command = form.command.at(0) === '/' ? form.command.slice(1) : form.command;
	...
	>
		<div class="my-2">
			...
			<div>
				<input
					...
					placeholder={$i18n.t('Add a short title for this form')}
					bind:value={title}
					required
				/>
			</div>
		...
		<div class="my-2">
			<div class="flex w-full justify-between">
				<div class=" self-center text-sm font-semibold">{$i18n.t('Form Content')}*</div>
			</div>
