<script>
	...
	import { getForms, updateFormByCommand } from '$lib/apis/forms';
	...
	const updateHandler = async () => {
		...
		if (validateCommandString(command)) {
			const form = await updateFormByCommand(localStorage.token, command, title, content).catch(
				(error) => {
					toast.error(error);
					return null;
				}
	...
	onMount(async () => {
		...
		if (command) {
			const form = $forms.filter((form) => form.command === command).at(0);
			...
			if (form) {
				...
				title = form.title;
				await tick();
				command = form.command.slice(1);
				content = form.content;
			} else {
				goto('/workspace/forms');
			}
		} else {
			goto('/workspace/forms');
		}
